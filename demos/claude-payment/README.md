...
2. **設定時 — demo:policy**：「上限金額（ETH）」に `0.1` などを入力します。既存の送金先・日次上限などを維持し、入力した上限でポリシー登録、commitment作成、Smart Account更新、activateまで実行します。完了出力はpolicyId、policyVersion、commitment、txHashです。
