...
const input = z
  ...
  .parse(process.env);
const recipient = z.string().regex(/^0x[0-9a-fA-F]{40}$/).parse(process.argv[2]);
const valueWei = z.string().regex(/^(0|[1-9][0-9]*)$/).parse(process.argv[3]);

const result = await payWithPolicyProof({
