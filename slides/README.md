...
| 4 | アーキテクチャ — ZK Policyのシステム構成 | サービス（2分20秒） |
...
4枚目（`.system-map`）は構成図として部品・責務・保持データとオフチェーン／オンチェーンの配置を示す。実線はMCP・証明API・Bundler・検証呼び出しの接続、破線はProverとVerifierが同じNoir回路に対応する関係（直接通信ではない）。Noir回路、証明方式UltraHonk、実装Barretenbergを証明基盤内に配置する。commitmentはAccountの保持データとして示し、秘密Policyとの関係を式で説明する。Ownerの設定権限と秘密の保持場所も明記する。処理順・設定手順は次のZKシーケンスで説明し、この図には番号を付けない。スタイルは専用の`.system-*`クラスに限定する。
