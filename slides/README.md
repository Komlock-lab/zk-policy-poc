...
図はmermaidを使わず、gridとflexのHTMLで組んでいる。ノード数が増えると自動レイアウトの矢印が交差して読めなくなったため、配置を固定した。図を足すときも同じ方針で書く。4枚目の図は`style.css`の`.policy-diagram`とアーキテクチャ用クラスで配置し、ラベルと矢印をHTMLのまま編集できる。
