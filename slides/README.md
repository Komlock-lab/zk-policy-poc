...
| 5 | Programmable Cryptography — 秘密の条件を証明し、実際の決済で検証する | Programmable Cryptography（1分） |
| 6 | デモ — 送金が進む経路と、止まる場所 | デモ（50秒） |
...
図はmermaidを使わず、gridとflexのHTMLで組んでいる。ノード数が増えると自動レイアウトの矢印が交差して読めなくなったため、配置を固定した。図を足すときも同じ方針で書く。4〜6枚目の図は`style.css`の`.policy-diagram`と各図専用クラスで配置し、ラベルと矢印をHTMLのまま編集できる。詳細な入力索引・実測値はプレゼンターノートに残す。
