...
`pnpm test:unit`のCLI 4件と`pnpm test:e2e`のPolicy更新scenarioが成功。CLIがPolicy ID、期待version、pending statusを検証し、署名済みCommitmentからcalldataを再構築してAPI応答との一致後に送信することを確認した。送信・receipt待機・active化失敗時のerrorにはpending versionと、取得済みの場合はtx hashを含める。
